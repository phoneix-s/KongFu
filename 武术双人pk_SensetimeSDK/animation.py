...
animation_start={'flip':False,
                    'sound':None,
                    #'frames':[{'body_points':[[134, 60], [134, 102], [111, 116], [157, 116], [93, 139], [176, 139], [120, 116], [157, 125], [125, 185], [148, 185], [120, 255], [157, 250], [125, 315], [157, 315]],'delay':10}]}
...
animation_flying_kick={'flip':False, 
                        ...
                        'frames':[  {'body_points':[[133, 67], [123, 111], [98, 116], [138, 121], [108, 141], [158, 161], [138, 116], [183, 156], [108, 191], [128, 191], [98, 256], [143, 256], [88, 320], [148, 315]],'delay':10},
                                    #{'body_points':[[37, 0], [48, 44], [26, 66], [70, 55], [32, 105], [92, 94], [54, 72], [114, 116], [70, 138], [92, 127], [70, 114], [126, 100], [70, 200], [126, 170]],'delay':100},
...
animation_list = [animation_standby,animation_kick, animation_flying_kick,animation_punch, animation_punch,animation_start,animation_stand]
# b=np.array([[133, 60], [133, 100], [104, 119], [158, 119], [99, 164], [163, 164], [94, 208], [167, 203], [113, 193], [143, 193], [108, 262], [148, 262], [113, 340], [148, 340]]) +np.array([00,-10]).astype(int)
# #b=(np.array([[133, 67], [123, 111], [98, 116], [138, 121], [108, 141], [158, 161], [138, 116], [183, 156], [108, 191], [128, 191], [98, 256], [143, 256], [88, 320], [148, 315]])+np.array([[19, -56], [34, -9], [8, 12], [65, -4], [24, 43], [97, 28], [50, 12], [76, 12], [76, 80], [97, 54], [30, 110], [170, 80], [100, 120], [242, 100]]))/np.array([2,2]).astype(int)
# b=b.astype(int)
# c=b.tolist()
# print(c)
...
def main():
    ...
    # 指定颜色
    rev_color = True  # True：蓝底白人   False:蓝人白底
    if rev_color:
        color = (255, 255, 255)
        bg_color = (100, 33, 3)
    else:
    ...
    for image in cam:
        ...
        image02 = np.zeros((image.shape[0], image.shape[1], 3), np.uint8)
        cv2.rectangle(image02, (0, 0), (image.shape[1], image.shape[0]),bg_color,thickness=-1)

        rect = body_detector.detect(image)
        if len(rect) > 0:
            points = body_aligner.align(image, rect[0])

            if points is not None:
                image02 = draw_stick_figure(
                    image02,  #背景图
                    points, #关键点
                    color=color,
                    bg_color=bg_color,
                )
            cv2.imshow("cartoon",image02)
        else:
            image02 = np.zeros((image.shape[0], image.shape[1], 3), np.uint8)
            cv2.putText(image02, "No human body detected", (int(image.shape[0]/4), int(image.shape[1]/3)),cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
            cv2.imshow("cartoon", image02)
...
if __name__ == '__main__':
    ...
    # player_pk_animator.player1_animation = animation_kick
    # player_pk_animator.player2_animation = animation_punch
