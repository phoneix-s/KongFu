...
def detect_body(frame):
    rects =  g_body_detector.detect(frame)
    if len(rects) >= 1:
        return rects[0]
    else:
        return [-1, -1, -1, -1]
...
def align_body(frame, rect):
    points = g_body_aligner.align(frame, rect)
    return points

def render_body_rect(frame,rect):
    rect_frame = g_body_detector.render(frame, rect)
    return rect_frame

def render_body_points(frame,points):
    points_frame = g_body_aligner.render(frame, points)
    return points_frame